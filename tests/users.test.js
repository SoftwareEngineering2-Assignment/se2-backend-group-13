...
test('POST /create create duplicate user', async (t) => {
  ...
  const newduplicateuser = {username: 'testuser5', password: 'testpassword6', email: 'otinanai@gmail.com'};
...
test('POST /authenticate user with wrong name', async (t) => {
  // give wrong username
  const wrongname = {username: 'kotsos7', password: 'password2000'};
...
test('POST /authenticate user with wrong password', async (t) => {
  // give wrong password
  const wrongpass = {username: 'kotsos2000', password: 'password7'};
...
test('POST /changepassword change password', async (t) => {
  ...
  const {body, statusCode} = await t.context.got.post(`users/changepassword?token=${token}&username=kotsos2000`, {json: changePass});
  // sc 200
  t.is(statusCode, 200);
...
test('POST /changepassword change password for a non existing user', async (t) => {
  ...
  const changePass = {password: 'password2000'};
  // body has the password , query has token and username
  const {body, statusCode} = await t.context.got.post(`users/changepassword?token=${token}&username=kotsos5`, {json: changePass});
  // sc 200
  t.is(statusCode, 200);
